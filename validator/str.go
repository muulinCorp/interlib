...
const (
	...
	rfc3339Regexp = `^([0-9]+)-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])[Tt]([01][0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9]|60)(\.[0-9]+)?(([Zz])|([\+|\-]([01][0-9]|2[0-3]):[0-5][0-9]))$`
)

var (
	rxEmail       = regexp.MustCompile(email)
	rxRfc3339     = regexp.MustCompile(rfc3339Regexp)
	ErrNotRfc3339 = errors.New("format is like " + time.RFC3339)
)
...
func IsRFC3339(timeStr string) error {
	if x := rxRfc3339.MatchString(timeStr); !x {
