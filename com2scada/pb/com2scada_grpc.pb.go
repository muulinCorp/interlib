...
type Com2ScadaServiceClient interface {
	...
	RoutineDataStream(ctx context.Context, in *FieldList, opts ...grpc.CallOption) (Com2ScadaService_RoutineDataStreamClient, error)
}
...
func (c *com2ScadaServiceClient) RoutineDataStream(ctx context.Context, in *FieldList, opts ...grpc.CallOption) (Com2ScadaService_RoutineDataStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Com2ScadaService_ServiceDesc.Streams[1], "/com2scada.Com2ScadaService/routineDataStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &com2ScadaServiceRoutineDataStreamClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type Com2ScadaService_RoutineDataStreamClient interface {
	Recv() (*ChangeDataResponse, error)
	grpc.ClientStream
}

type com2ScadaServiceRoutineDataStreamClient struct {
	grpc.ClientStream
}

func (x *com2ScadaServiceRoutineDataStreamClient) Recv() (*ChangeDataResponse, error) {
	m := new(ChangeDataResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type Com2ScadaServiceServer interface {
	...
	RoutineDataStream(*FieldList, Com2ScadaService_RoutineDataStreamServer) error
...
func (UnimplementedCom2ScadaServiceServer) RoutineDataStream(*FieldList, Com2ScadaService_RoutineDataStreamServer) error {
	return status.Errorf(codes.Unimplemented, "method RoutineDataStream not implemented")
}
...
func _Com2ScadaService_RoutineDataStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(FieldList)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(Com2ScadaServiceServer).RoutineDataStream(m, &com2ScadaServiceRoutineDataStreamServer{stream})
}

type Com2ScadaService_RoutineDataStreamServer interface {
	Send(*ChangeDataResponse) error
	grpc.ServerStream
}

type com2ScadaServiceRoutineDataStreamServer struct {
	grpc.ServerStream
}

func (x *com2ScadaServiceRoutineDataStreamServer) Send(m *ChangeDataResponse) error {
	return x.ServerStream.SendMsg(m)
}
...
var Com2ScadaService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "routineDataStream",
			Handler:       _Com2ScadaService_RoutineDataStream_Handler,
			ServerStreams: true,
		},
	},
